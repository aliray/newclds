...
from datasets.datasets import CassavaDataset
...
CFG = {
    ...
    'folds': [2],
    'weights': [1, 1, 1, 1],
    'expid': f'ef4_cutmix_fold2_{time.strftime("%Y-%m-%d-%H:%M:%S", time.localtime())}'
}
...
if __name__ == '__main__':
    ...
    for fold, (trn_idx, val_idx) in enumerate(folds):
        ...
        train_ds = CassavaDataset(train_, image_root, img_size=CFG['img_size'], transforms=get_train_transforms(),
                                  do_cutmix=CFG['do_cutmix'], do_fmix=CFG['do_fmix'], one_hot_label=True)
        valid_ = train.loc[val_idx, :].reset_index(drop=True)
        valid_ds = CassavaDataset(valid_, image_root, img_size=CFG['img_size'], transforms=get_valid_transforms(),
                                  one_hot_label=True)
        import math
        ...
        for epoch in range(CFG.get('epochs')):
            ...
            for phase in ['train', 'val']:
                ...
                print('data loader len is ', len(dataloaders[phase]))
                running_loss = 0.0
                running_corrects = 0

                # Iterate over data.
                lr = optimizer.state_dict()["param_groups"][0]["lr"]
                for step, (inputs, labels, one_hot_label) in enumerate(dataloaders[phase]):
                    ...
                    with autocast():
                        with torch.set_grad_enabled(phase == 'train'):
                            ...
                            loss = criterion(outputs, one_hot_label, t1=CFG['t1'], t2=CFG['t2'])
                            if phase == 'train':
