...
from loss.elr_loss import ElrLoss

CFG = {
    'fold_num': 5,
    'seed': 719,
    'model_arch': 'resnet34_elr_loss',
    'img_size': 512,
    'epochs': 100,
    'train_bs': 32,
    'val_bs': 32,
    ...
    'weights': [1, 1, 1, 1],
    'expid': f'resnet34_elr_loss_{time.strftime("%Y-%m-%d-%H:%M:%S", time.localtime())}'
}
...
if __name__ == '__main__':
    ...
    for fold, (trn_idx, val_idx) in enumerate(folds):
        ...
        criterion = ElrLoss(len(train_ds), train.label.nunique(), lmd=0.9)
        optimizer = optim.Adam(model.parameters(), lr=CFG['lr'], weight_decay=CFG['weight_decay'])
        exp_lr_scheduler = lr_scheduler.ReduceLROnPlateau(
            optimizer, factor=0.9, patience=1, min_lr=1e-6, verbose=True
        )
        model.train()
        dataloaders = {
            'train': train_loader,
        ...
        for epoch in range(CFG.get('epochs')):
            ...
            for phase in ['train', 'val']:
                ...
                for step, (inxs, inputs, labels) in enumerate(dataloaders[phase]):
                    ...
                    with autocast():
                        with torch.set_grad_enabled(phase == 'train'):
                            ...
                            loss = criterion(inxs, outputs, labels)
                            if phase == 'train':
                                # loss.backward()
