...
def get_img(path):
    ...
    im_rgb = im_bgr[:, :, ::-1]
    im_rgb = np.asarray(im_rgb).transpose(-1, 0, 1)
    return im_rgb
...
if __name__ == '__main__':
    ...
    model = UnetClassifier(train.label.nunique()).to(device)
    ...
    for fold, (trn_idx, val_idx) in enumerate(folds):
        ...
        if not CFG.get('is_test', False):
                ...
                for phase in ['train', 'val']:
                    ...
                    for step, (inputs, labels) in enumerate(dataloaders[phase]):
                        ...
                        if step % CFG.get('verbose_step') == 0:
                            print(f'fold {fold} {phase} step {step} running step loss {step_loss} step acc {step_acc}')
                    ...
                    # deep copy the model
                    if phase == 'val':
                        torch.save(model.state_dict(), f'./chk/{CFG.get("model_arch")}_{fold}_{epoch}.pt')
                        if epoch_acc > best_acc:
                            best_acc = epoch_acc
                            torch.save(model.state_dict(), f'./chk/{CFG.get("model_arch")}_best_model_{fold}.pt')
            ...
            valid_ds = CassavaDataset(valid_, image_root, transforms=get_valid_transforms(), output_label=False)
            val_loader = torch.utils.data.DataLoader(
                valid_ds,
                batch_size=CFG['valid_bs'],
                num_workers=CFG['num_workers'],
