...
CFG = {
    ...
    'weights': [1, 1, 1, 1],
    'expid': f'densenet121_{time.strftime("%Y-%m-%d-%H:%M:%S", time.localtime())}'
}
...
if __name__ == '__main__':
    ...
    for fold, (trn_idx, val_idx) in enumerate(folds):
        ...
        exp_lr_scheduler = lr_scheduler.CosineAnnealingLR(optimizer=optimizer, T_max=10, eta_min=1e-6)
        model.train()
        dataloaders = {
            'train': train_loader,
            'val': val_loader
