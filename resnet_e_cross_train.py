...
CFG = {
    ...
    'weights': [1, 1, 1, 1],
    'expid': f'resnet_consine_{time.strftime("%Y-%m-%d-%H:%M:%S", time.localtime())}'
}
            ...
            for phase in ['train', 'val']:
                ...
                for step, (inputs, labels) in enumerate(dataloaders[phase]):
                    ...
                    with autocast():
                        ...
                        if step % CFG.get('verbose_step') == 0 and step != 0:
                            print(
                                f'fold {fold} {phase} step {step} running step lr '
                                f'{lr} '
                                f'step loss {step_loss} step acc {step_acc}')
                            writer.add_scalar(f'{phase} loss', step_loss, (epoch + 1) * step)
