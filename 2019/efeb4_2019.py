...
from albumentations import (
    ...
    ShiftScaleRotate, CenterCrop, Resize, RandomShadow
)
...
class CassvaImgClassifier(nn.Module):
    def __init__(self, model_arch, n_class, drop_rate=0, drop_path_rate=0, pretrained=True):
        super().__init__()
        self.model = timm.create_model(model_arch, drop_rate=drop_rate, drop_path_rate=drop_path_rate,
...
def train(CFG):
    ...
    for fold, (trn_idx, val_idx) in enumerate(folds):
        ...
        model = CassvaImgClassifier(model_arch, 5, drop_rate=CFG['drop_rate'], drop_path_rate=CFG['drop_path_rate']).to(
            device)
        if CFG['loading_pre']:
            model.load_state_dict(torch.load(CFG['pre_path']))
        scaler = GradScaler()
    ...
    confs = [
        {
            ...
            'folds': [0],
            'loading_pre': False,
            'pre_path': '/home/gpuserver/race/race_clds/2019/chk/efb4_2019_2021-02-03-17:02:08/'
                        'tf_efficientnet_b4_ns_best.pt',
            'lrstep': True,
            'label_smoothing': 0.0,
            'drop_rate': 0.2,
            'drop_path_rate': 0.0,
            'expid': f'e4_dropre_{time.strftime("%Y-%m-%d-%H:%M:%S", time.localtime())}',
        }
    ]
    print(confs)
    train(confs[0])
