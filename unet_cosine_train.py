...
CFG = {
    ...
    'weight_decay': 1e-6,
    'num_workers': 4,
    'accum_iter': 8,  # suppoprt to do batch accumulation for backprop with effectively larger batch size
    'verbose_step': 10,
    'device': 'cuda:0',
    'tta': 3,
    'used_epochs': [43, 45, 46, 69],
    'weights': [1, 1, 1, 1],
    'expid': f'best_lr_{time.strftime("%Y-%m-%d-%H:%M:%S", time.localtime())}'
}
...
class CassvaImgClassifier(nn.Module):
    def __init__(self, model_arch, n_class, pretrained=True):
        super().__init__()
        from pl_bolts.models.vision import PixelCNN, UNet
        self.model = UNet(5)
