...
CFG = {
    ...
    'load2019': False,
    'expid': f'regnetx_032_{time.strftime("%Y-%m-%d-%H:%M:%S", time.localtime())}'
}
...
if __name__ == '__main__':
    ...
    expid = CFG.get('expid')
    exp_root = '/home/gpuserver/race/exps'
    tb_path = f'{exp_root}/tb/{expid}/{time.strftime("%Y-%m-%d-%H:%M:%S", time.localtime())}'
    chk_root_path = f'{exp_root}/chk/{expid}/{time.strftime("%Y-%m-%d-%H:%M:%S", time.localtime())}'
    ...
    for fold, (trn_idx, val_idx) in enumerate(folds):
        ...
        for epoch in range(CFG.get('epochs')):
            ...
            for phase in ['train', 'val']:
                ...
                print(f'fold {fold} {phase} loss {epoch_loss} acc {epoch_acc} cost {time.time() - stime}')
