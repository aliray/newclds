...
from loss.rceloss import NCEandRCE, NFLandRCE, NFLandNCE, NFLandMAE, NGCEandRCE, NGCEandNCE, NGCEandMAE
...
CFG = {
    ...
    'expid': f're50_rce_{time.strftime("%Y-%m-%d-%H:%M:%S", time.localtime())}'
}
...
if __name__ == '__main__':
    ...
    for fold, (trn_idx, val_idx) in enumerate(folds):
        ...
        criterion = NGCEandMAE(alpha=CFG['alpha'], beta=CFG['beta'], num_classes=CFG['num_classes'])
        exp_lr_scheduler = \
            lr_scheduler.CosineAnnealingWarmRestarts(optimizer, T_0=CFG['T_0'], T_mult=1,
                                                     last_epoch=-1)
