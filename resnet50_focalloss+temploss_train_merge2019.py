...
CFG = {
    ...
    'weights': [1, 1, 1, 1],
    'expid': f'resnet50_focalloss_{time.strftime("%Y-%m-%d-%H:%M:%S", time.localtime())}'
}
...
if __name__ == '__main__':
    ...
    for fold, (trn_idx, val_idx) in enumerate(folds):
        ...
        exp_lr_scheduler = lr_scheduler.ReduceLROnPlateau(optimizer, factor=0.9, patience=3, min_lr=1e-6)
        model.train()
        dataloaders = {
            'train': train_loader,
            'val': val_loader
                ...
                for step, (inputs, labels) in enumerate(dataloaders[phase]):
                    ...
                    with autocast():
                        with torch.set_grad_enabled(phase == 'train'):
                            ...
                            loss = criterion(outputs, labels) + bi_tempered_logistic_loss(outputs, labels, 0.2, 1.3)
                            if phase == 'train':
                                # loss.backward()
