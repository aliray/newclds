...
from loss.tempered_loss import bi_tempered_logistic_loss, bi_tempered_binary_logistic_loss
...
CFG = {
    ...
    'weights': [1, 1, 1, 1],
    'expid': f'ef4+temp+smooth_{time.strftime("%Y-%m-%d-%H:%M:%S", time.localtime())}'
}
                    ...
                    with autocast():
                        ...
                        if step % CFG.get('verbose_step') == 0:
                            print(
                                ...
                                f'{lr} '
                                f'step {(epoch + 1) * (step + 10)} loss {step_loss} step acc {step_acc}')
