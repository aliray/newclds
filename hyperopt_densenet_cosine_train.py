...
CFG = {
    ...
    'weights': [1, 1, 1, 1],
    'expid': f'hyperopt_{time.strftime("%Y-%m-%d-%H:%M:%S", time.localtime())}'
}
                ...
                for step, (inputs, labels) in enumerate(dataloaders[phase]):
                    ...
                    with autocast():
                        with torch.set_grad_enabled(phase == 'train'):
                            ...
                            loss = criterion(outputs, labels, t1=t1, t2=t2, label_smoothing=args['label_smoothing'])
                            if phase == 'train':
...
if __name__ == '__main__':
    ...
    spaces = hp.choice('classifier', [{
        ...
        't1': 0.9,
        't2': 1.8,
        'label_smoothing': hp.choice('t1', [0.1, 0.15, 0.2, 0.25, 0.3, 0.35, 0.4, 0.45, 0.5])
    }])
    best = fmin(
        train_fn,
        space=spaces,
